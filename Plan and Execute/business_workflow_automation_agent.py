...
def validate_expense_report(report_id:str) -> str:
    """validate an employees expense report"""
    ...
    try:
        ...
        return f"Error validating report {report_id}: {str(e)}"
@tool
def check_policy_compliance(report_id:str) -> str:
    """Check if the expense report complies with company policy"""
    if not report_id or not isinstance(report_id, str):
        return "Error: Invalid report ID provided."
    try:
        return f"Expense report {report_id} complies with company policy."
    except Exception as e:
        return f"Error checking compliance for report {report_id}: {str(e)}"
@tool
def route_to_manager(report_id:str, manager_id:str) -> str:
    """Route the expense report to the manager for approval"""
    if not report_id or not isinstance(report_id, str):
        return "Error: Invalid report ID provided."
    try:
        return f"Expense report {report_id} routed to manager {manager_id} for approval."
    except Exception as e:
        return f"Error routing report {report_id} to manager {manager_id}: {str(e)}"
@tool
def notify_employee(report_id:str, status:str) -> str:
    """Notify the employee about the status of their expense report"""
    if not report_id or not status:
        return "Error: Invalid report ID or status provided."
    try:
        return f"Employee notified about the status of report {report_id}: {status}."
    except Exception as e:
        return f"Error notifying employee about report {report_id}: {str(e)}"
...
class Act(BaseModel):
    action: Union[Response, Plan] = Field(description="Action to take")

# Improved system prompts
SYSTEM_PROMPT = """You are an expense report processing assistant. Your task is to
validate and process expense reports
using the available tools. Always extract the report ID from the input and use it consistently
across all steps.
Available tools:
...
def create_workflow():
    ...
    "replan",
    lambda s: END if ("response" in s or "error" in s) else "agent", [END]
    )
